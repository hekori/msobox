...
testing enviroment for optimal control with single shooting...
...
# local imports
from ocss_snopt import OCSS_snopt
from ocss_indegrator import OCSS_indegrator
from ocss_sensitivity import OCSS_sensitivity
...
if __name__ == "__main__":
    ...
    # problem     = OCSS_indegrator(path="./examples/chemie/", minormax="max", NX=2, NG=0, NP=1, NU=1, bc=bc, ts=ts)
    # x0          = [1, 0]
    # xend        = [None, None]
    # p           = np.array([3.5])
    ...
    problem     = OCSS_indegrator(path="./examples/splines2/", minormax="min", NX=3, NG=1, NP=1, NU=1, bc=bc, ts=ts)
    x0          = [0, 1, 0]
    xend        = [0, -1, None]
    p           = np.array([3])
    q0          = 0 * np.ones((problem.NQ,))
    ...
    # ts          = np.linspace(0, 1, 25)
    # bc          = np.array([-6, 6], ndmin=2)
    # problem     = OCSS_indegrator(path="./examples/splines1/", minormax="min", NX=3, NG=0, NP=1, NU=1, bc=bc, ts=ts)
    # x0          = [0, 1, 0]
    # xend        = [0, 1, None]
    # p           = np.array([3])
    # q0          = 0 * np.ones((problem.NQ,))
    ...
    pl.savefig("output/" + datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S") + "-plot.png", bbox_inches="tight")
    pl.savefig("output/" + datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S") + "-plot.pdf", bbox_inches="tight")
    pl.show()
