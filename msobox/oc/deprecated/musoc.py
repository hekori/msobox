...
class MUSOC(object):
    ...
    def __init__(self, dt=0.05, M=20, **kwargs):
        ...
        self.NH  = NH         # number of all functios of all processes
        ...
        self.q     = numpy.zeros((self.NU, self.M, 2))

         # values at time node 0
        self.h = numpy.zeros(ex.NM)

        # temporary variables for derivative arrays
        ...
        self.Vxd = numpy.zeros((P, D, self.xd.size))
        self.Vxa = numpy.zeros((P, D, self.xa.size))
    ...
    def update_arrival_cost(self):
        ...
        self.etas[0, :]
        self.sigmas[0, :]
        are incorporated into the arrivale cost.
        ...
        xdp, xap, Vxdp, Vxap = self.integrator.forward(
                                      [t0, tend],
                                      self.xd, self.xa, self.q.ravel(), self.p,
        ...
        # store Xx
        Xx[:self.NY, :] = Vxdp[:self.NX, 0, :].T
        Xx[self.NY:, :] = Vxap[:self.NX, 0, :].T

        # store Xp
        Xp[:self.NY, :] = Vxdp[self.NX:, 0, :].T
        Xp[self.NY:, :] = Vxap[self.NX:, 0, :].T

        # print 'self.Vxd, self.Vxa, self.Vq, self.Vp=\n',self.Vxd, self.Vxa, self.Vq, self.Vp
        # print 'self.xd, self.xa, self.q.ravel(), self.p =\n',self.xd, self.xa, self.q.ravel(), self.p
        ...
        htilde[:] = 0.
        Hx = numpy.zeros((self.NH, self.NX))
        Hp = numpy.zeros((self.NH, self.NP))

        # tmp variables to compute Jacobians
        eyeNX = numpy.eye(self.NX)
        eyeNP = numpy.eye(self.NP)
    ...
    def _preparation_phase_integration(self, ci):
        ...
        # perform integration
        self.xd[:], self.xa[:], self.Vxd[...], self.Vxa[...] = \
    ...
    def simulate_measurement(self, simulate_error=False):
        ...
        * self.sigmas
        ...
        for j in range(self.M):
            ...
            self.xd[...], self.xa[...] = self.integrator.forward(
                [t0, tend],
                self.xd, self.xa, self.q.ravel(), self.p_ref)
